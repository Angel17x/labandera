(function(app) {
	...
	app.AppCallService.prototype.callServicesHttp=function(ser,querys,param){
		...
		switch(ser){
			...
			case 'travel-report-available':{
				request=this.callServices(this.enlace_api+'_open/trip/report?realm='+this.getRealm()+querys,"POST",param,null,false,"JSON","application/json",true,null);
				return request;
			}break;
			case 'get-exchanged':{
	...
	app.AppCallService.prototype.setCurrencies=function(data){
		localStorage.setItem('currencies', data);
	}
	app.AppCallService.prototype.getCurrencies=function(){
		var aux=null;
		try{
			aux=localStorage.getItem("currencies");
		}catch(er){
			aux=null;
		}
		return aux;
